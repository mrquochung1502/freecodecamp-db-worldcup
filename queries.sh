...
echo "$($PSQL "SELECT GREATEST(MAX(winner_goals), MAX(opponent_goals)) AS most_goal_scored FROM games")"

echo -e "\nNumber of games where the winning team scored more than two goals:"
echo "$($PSQL "SELECT COUNT(*) AS scored_more_than_two_goals FROM games WHERE winner_goals > 2")"
...
(SELECT name FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE round = 'Eighth-Final' AND year = 2014
UNION
SELECT name FROM games INNER JOIN teams ON games.opponent_id = teams.team_id WHERE round = 'Eighth-Final' AND year = 2014)
AS eighth_finalists
ORDER BY name
")"
